      ...
      for (let i = 20; i < boundingBoxParent.width; i += 20)
        {
          for (let j = 20; j < boundingBoxParent.height; j += 20)
          {
            ...
            if (trigger)
            {
              let x = round(trigger.x, 20, 0);
              let y = round(trigger.y, 20, 0);
              let mainX = round(boundingBoxParent.x + j, 20, 0);
              let mainY = round(boundingBoxParent.y + i, 20, 0);
              if (mainX == x && mainY == y)
              {
            ...
            else ellipse.style.background = "blue";
