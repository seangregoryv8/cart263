class DrawingBoard
{
    ...
    constructor(canvas, context,drawingBoardId)
    {
        ...
        this.canvas.addEventListener("click", e => self.clickCanvas(e));
        this.canvas.addEventListener("mousemove", e => self.overCanvas(e));
    ...
    updateMouseHistory(e)
    {
        ...
        if (this.prevTime !== 0)
        {
            ...
            this.direction = { x: dx / dt, y: dy / dt };
        }
    ...
    overCanvas(e)
    {
        ...
        if(this.drawingBoardId === "partB") this.initAudio();
        if(this.drawingBoardId ==="partC") this.initAudio();
        if(this.drawingBoardId ==="partD") this.objectsOnCanvas[0].updatePositionRect(this.mouseOffsetX, this.mouseOffsetY);
    }
    ...
    clickCanvas(e)
    {
        ...
        if(this.drawingBoardId ==="partB") this.processAudio();
        if(this.drawingBoardId ==="partC") this.processAudio();
        if(this.drawingBoardId ==="partD") this.objectsOnCanvas[0].changeColor(this.getRandomColor());
    }

    /* method to add obj to canvas */
    addObj(objToAdd) { this.objectsOnCanvas.push(objToAdd); }

    /* method to add display objects on canvas */
    display() { for (let obj of this.objectsOnCanvas) obj.display() }
